...
def rad2deg(x): #rad->degree
    return x*180/np.pi
def s_list(loc_lat,loc_long,sol_t,diff,off1): # 위도, 경도, 태양시, 항성시-태양시, 자전축 돌릴 각도
    ...
    for star in star_eq:
        ...
        if sina>np.sin(5.*np.pi/180) and star[1]<3.5:#특정 밝기 이상, 천정거리 이상의 별만 선별
    ...
    missing=np.random.choice(len(star_visA),5,replace=False) #임의의 다섯 개의 별 삭제
    star_visA0 = np.array(star_visA)
    missinglist = star_visA0[missing]
    missingstars = []
    for i in missinglist:
        missingstars.append(int(i[0]))
    return star_vis, missing, missingstars
...
print('latitude(n / s / r / #), longitude(r / #), time(r / #), number(#)')
ans = input('input: ')
lat_ans = ans.split()[0]
...
# #save problems
  ...
  a,b,c = s_list(lat, long, solar_time, 3.35, offset)
